...
func TestStreamChatRejectsOversizedAccumulatedToolCallName(t *testing.T) {
	t.Parallel()

	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "text/event-stream")
		_, _ = fmt.Fprint(w, "data: {\"choices\":[{\"index\":0,\"delta\":{\"tool_calls\":[{\"index\":0,\"id\":\"call_1\",\"type\":\"function\",\"function\":{\"name\":\"tool_\",\"arguments\":\"{}\"}}]}}]}\n\n")
		chunk := strings.Repeat("a", 128*1024)
		for range 9 {
			_, _ = fmt.Fprintf(w, "data: {\"choices\":[{\"index\":0,\"delta\":{\"tool_calls\":[{\"index\":0,\"function\":{\"name\":%q}}]}}]}\n\n", chunk)
		}
		_, _ = fmt.Fprint(w, "data: [DONE]\n\n")
	}))
	defer srv.Close()
	...
	if !strings.Contains(err.Error(), "tool call name exceeds") {
		t.Fatalf("StreamChat error = %q, want tool-call name limit error", err)
	}
}
