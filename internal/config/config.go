...
func Load(path string) (Config, error) {
	...
	for _, r := range cfg.Arena.APIKeyEnv {
		...
		if r == '\u2028' || r == '\u2029' {
			return Config{}, errors.New("arena.apiKeyEnv must not contain line separators")
		}
		if r >= '\u202a' && r <= '\u202e' || r >= '\u2066' && r <= '\u2069' {
			return Config{}, errors.New("arena.apiKeyEnv must not contain bidirectional formatting")
		}
		if unicode.Is(unicode.Cf, r) {
			return Config{}, errors.New("arena.apiKeyEnv must not contain Unicode format characters")
		}
	}
	...
	for _, r := range cfg.Arena.Model {
		...
		if r == '\u2028' || r == '\u2029' {
			return Config{}, errors.New("arena.model must not contain line separators")
		}
		if r >= '\u202a' && r <= '\u202e' || r >= '\u2066' && r <= '\u2069' {
			return Config{}, errors.New("arena.model must not contain bidirectional formatting")
		}
		if unicode.Is(unicode.Cf, r) {
			return Config{}, errors.New("arena.model must not contain Unicode format characters")
		}
	}
	for i, fallback := range cfg.Arena.Fallbacks {
		...
		for _, r := range fallback {
			...
			if r == '\u2028' || r == '\u2029' {
				return Config{}, fmt.Errorf("arena.fallbacks[%d] must not contain line separators", i)
			}
			if r >= '\u202a' && r <= '\u202e' || r >= '\u2066' && r <= '\u2069' {
	...
	for name, server := range cfg.MCPServers {
		...
		if trimmedName == "" {
			return Config{}, errors.New("mcpServers server name must not be empty")
		}
		if trimmedName != name {
			return Config{}, errors.New("mcpServers server name must not have surrounding whitespace")
		}
		for _, r := range name {
			if unicode.IsControl(r) {
