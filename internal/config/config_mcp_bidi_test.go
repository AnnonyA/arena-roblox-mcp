...
func TestLoadRejectsBidirectionalFormattingInMCPProcessConfig(t *testing.T) {
	tests := []struct {
		name string
		data string
		want string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := Load(path)
			if err == nil {
				t.Fatal("Load error = nil, want bidirectional-formatting validation error")
			}
			if !strings.Contains(err.Error(), tt.want) {
				t.Fatalf("Load error = %q, want validation path %q", err, tt.want)
			}
			if !strings.Contains(err.Error(), "bidirectional formatting") {
				t.Fatalf("Load error = %q, want bidirectional-formatting validation error", err)
			}
		})
	}
}
