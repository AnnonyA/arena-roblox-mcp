...
func TestSaveJournalRejectsSymlinkedSessionDirectoryAncestor(t *testing.T) {
	...
	path := filepath.Join(linkedParent, "sessions", "session.json")
	err := SaveJournal(path, NewJournal())
	if err == nil {
		t.Fatal("SaveJournal() error = nil, want symlinked ancestor rejection")
	}
	if !strings.Contains(err.Error(), "symbolic links") {
		t.Fatalf("SaveJournal() error = %q, want symbolic-link rejection", err)
	}
	if _, statErr := os.Stat(filepath.Join(outside, "sessions", "session.json")); !os.IsNotExist(statErr) {
