...
func boundedSafeDisplayText(text string, maxRunes int) string { safe:=safeDisplayText(text); runes:=[]rune(safe); if maxRunes<=0 { return "" }; if len(runes)<=maxRunes { return safe }; if maxRunes==1 { return "…" }; return string(runes[:maxRunes-1])+"…" }
...
func writeText(out io.Writer, text string) error {
	...
	if err == nil && n != len(text) { return io.ErrShortWrite }
	return err
}
...
func NewCommandHandlerWithActions(out io.Writer, actions CommandActions, next InputHandler) InputHandler {
	...
	return func(ctx context.Context,input Input)(bool,error){
		...
		if input.Kind==InputCommand&&input.Command=="models"&&actions.Models!=nil {models,err:=actions.Models(ctx);if err!=nil{return false,err};models=safeModelIDs(models);arenaConnected=true;if len(models)==0{return false,writeText(out,"No Arena models available.\n")};return false,writeText(out,strings.Join(models,"\n")+"\n")}
		if input.Kind==InputCommand&&input.Command=="model"&&strings.TrimSpace(input.Argument)==""&&actions.Models!=nil {models,err:=actions.Models(ctx);if err!=nil{if actions.Model!=nil{return false,actions.Model(ctx,input.Argument)};return false,err};models=safeModelIDs(models);arenaConnected=true;if len(models)==0{return false,writeText(out,"No Arena models available.\n")};if err=writeText(out,"Select a model with /model <id>:\n");err!=nil{return false,err};return false,writeText(out,strings.Join(models,"\n")+"\n")}
		...
		if input.Kind==InputCommand&&input.Command=="tools"&&actions.Tools!=nil {tools,err:=actions.Tools(ctx);if err!=nil{return false,err};if len(tools)==0{return false,writeText(out,"No MCP tools available.\n")};safeTools:=make([]string,len(tools));for i:=range tools{safeTools[i]=boundedSafeDisplayText(tools[i],maxToolDisplayRunes)};return false,writeText(out,strings.Join(safeTools,"\n")+"\n")}
		if input.Kind==InputCommand&&input.Command=="history"&&actions.History!=nil {history,err:=actions.History(ctx);if err!=nil{return false,err};if len(history)==0{return false,writeText(out,"No session history recorded.\n")};safeHistory:=make([]string,len(history));for i:=range history{safeHistory[i]=safeDisplayText(history[i])};return false,writeText(out,strings.Join(safeHistory,"\n")+"\n")}
