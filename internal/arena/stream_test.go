...
func TestStreamChatAcceptsCumulativeToolArguments(t *testing.T) {
	t.Parallel()
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "text/event-stream")
		_, _ = w.Write([]byte("data: {\"choices\":[{\"delta\":{\"tool_calls\":[{\"index\":0,\"id\":\"call_1\",\"type\":\"function\",\"function\":{\"name\":\"script_read\",\"arguments\":\"{\\\"path\\\":\\\"A\"}}]}}]}\n\n"))
	...
	if got.ToolCalls[0].Function.Arguments != `{"path":"A"}` {
		t.Fatalf("tool arguments = %q, want %q", got.ToolCalls[0].Function.Arguments, `{"path":"A"}`)
	}
}
...
func TestStreamChatRejectsNegativeToolCallIndex(t *testing.T) {
	t.Parallel()
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "text/event-stream")
		_, _ = w.Write([]byte("data: {\"choices\":[{\"delta\":{\"tool_calls\":[{\"index\":-1,\"id\":\"call_bad\",\"type\":\"function\",\"function\":{\"name\":\"script_read\",\"arguments\":\"{}\"}}]}}]}\n\n"))
		_, _ = w.Write([]byte("data: [DONE]\n\n"))
	}))
	defer srv.Close()
	...
	if err == nil {
		t.Fatal("StreamChat error = nil, want negative tool-call index error")
	}
}
